...
public class StartupProgressDialog extends JFrame {
    ...
    public StartupProgressDialog(ZCashClientCaller clientCaller)
    {
        ...
        URL iconUrl = this.getClass().getClassLoader().getResource("images/ACASH.png");
    ...
    public void waitForStartup() throws IOException,
        ...
        Log.info("Splash: checking if acashd is already running...");
        ...
        if (!shouldStartZCashd) {
        	Log.info("Splash: acashd already running...");
            // What if started by hand but taking long to initialize???
//            doDispose();
//            return;
        } else
        {
        	Log.info("Splash: acashd will be started...");
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
            	Log.info("Stopping acashd because we started it - now it is alive: " +
                ...
                try
                {
	                ...
	                while (!StartupProgressDialog.this.waitFor(daemonProcess, 3000))
	                {
	                	...
	                	Log.info("Waiting for " + ((end - start) / 1000) + " seconds for acashd to exit...");
	                ...
	                if (StartupProgressDialog.this.isAlive(daemonProcess)) {
	                	Log.info("acashd is still alive although we tried to stop it. " +
	                                           "Hopefully it will stop later!");
	                    } else
	                    	Log.info("acashd shut down successfully");
                } catch (Exception bad) {
                	Log.error("Couldn't stop acashd!", bad);
                }
            }
        });
